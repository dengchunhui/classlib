...
其他
	...
	第三、在iframe中调用父页面中定义
	//四舍五入
	{function name="round" v=$deal.progress_point f=1}
	
	//URL跳转
	function skip_url($url,$parameter =array()){
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,$url);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 120); //设置连接时间
		curl_setopt($ch, CURLOPT_TIMEOUT, 120); //设置执行时间
